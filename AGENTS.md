...
- Execution Safeguards: do not create or modify tests unless requested; do not build, run, debug, or inspect logs unless requested; do not commit or push to git unless requested; never run destructive commands without explicit confirmation.
...
- Git & Commit Hygiene: when asked to commit, follow Conventional Commits (`feat:`, `fix:`, `refactor:`, `chore:`); keep commits atomic; never stage secrets, build artifacts, or temporary files.
