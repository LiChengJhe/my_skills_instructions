...
- Performance: avoid `SELECT *`; specify explicit column lists; ensure predicates in `WHERE` and `JOIN` are sargable (avoid wrapping indexed columns in functions like `YEAR(Date)` or `ISNULL()`).
